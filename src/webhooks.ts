...
export const stripeWebhookHandler = async (req: express.Request, res: express.Response) => {
  ...
  if (!session?.metadata?.userId || !session?.metadata?.orderId) {
    console.error('Missing userId or orderId in session metadata:', session.metadata);
    return res.status(400).send(`Webhook Error: No user or order present in metadata`);
  }
